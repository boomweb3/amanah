...
import { LedgerEntry, TransactionType, TransactionStatus, Direction, User } from '../types/types';
...
const LedgerForm: React.FC = () => {
  ...
  const otherUsers = useMemo(() => {
    ...
    return users.filter(u => (u._id || u.id) !== currentUserId);
  }, [users, user]);
  ...
  const selectedUser = useMemo(() => 
    otherUsers.find(u => u._id === selectedUserId || u.id === selectedUserId), [otherUsers, selectedUserId]
  );
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-72 overflow-y-auto pr-2 scrollbar-thin">
                  {filteredUsers.map(u => (
                    <button
                      key={u._id || u.id}
                      type="button"
                      onClick={() => setSelectedUserId(u._id || u.id || null)}
                    ...
                    >
                      <div className={`w-14 h-14 rounded-2xl ${u.avatar || 'bg-slate-200'} flex items-center justify-center text-white font-black text-lg mb-3 shadow-lg group-hover:scale-110 transition-transform`}>
                        {u.name.charAt(0)}
                      </div>
                      <span className="font-bold text-slate-900 text-sm">{u.name}</span>
                      <span className="text-[10px] text-slate-400 truncate w-full text-center">{u.email}</span>
                    </button>
                  ))}
